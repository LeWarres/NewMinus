...
import { MangaPreviewCoverComponent } from './components/manga-preview-cover/manga-preview-cover.component';
import { MangaPreviewChaptersComponent } from './components/manga-preview-chapters/manga-preview-chapters.component';
import { MangaPreviewInfoComponent } from './components/manga-preview-info/manga-preview-info.component';
import { SubscriptionChange } from '../../components/subscribe-button/subscribe-button.component';
...
@Component({
  ...
  imports: [
    CommonModule,
    MangaPreviewCoverComponent,
    MangaPreviewChaptersComponent,
    MangaPreviewInfoComponent,
...
export class MangaPreviewComponent implements OnInit {
  ...
  cargarPreview(id: string, idioma?: string): void {
    ...
    this.http
      .get<ObraPreviewResponse>(this.buildPreviewUrl(id, idioma), { withCredentials: true })
      .subscribe({
        next: (res) => {
          this.cargando = false;
  ...
  registrarVista(obraId: number): void {
    this.http.post(this.registrarVistaUrl, { obra_id: obraId }, { withCredentials: true })
      .subscribe({
        next: () => {},
        error: (err) => console.error('No se pudo registrar vista de la obra', err)
      });
  }
  ...
  abrirCapitulo(capitulo: Capitulo): void {
    ...
    this.router.navigate(
      ['/obra', this.obra.id, 'capitulo', capitulo.numeroCapitulo],
      { queryParams: { idioma: this.selectedLanguage } }
    );
  }

  leerPrimerCapitulo(): void {
    if (this.obra?.capitulos.length) {
  ...
  abrirIdiomaObra(idioma?: string): void {
    this.router.navigate(['/categorias'], {
      queryParams: {
        idioma: this.normalizarIdiomaBusqueda(idioma),
        nsfw: 'incluir'
      }
    });
  }
  ...
  getIdiomasDisponibles(): IdiomaPreviewItem[] {
    ...
    return Array.from(new Set(
      idiomas
        ...
        .filter(Boolean)
    )).map(value => ({ value, label: this.getIdiomaLabel(value) }));
  }
  ...
  getCategoriasObra(): string[] {
    ...
    if (this.obra.genero) {
      categorias.push(...this.obra.genero.split(',').map(item => item.trim().toLowerCase()).filter(Boolean));
    }

    if (this.obra.categorias?.length) {
      categorias.push(...this.obra.categorias.map(categoria => String(categoria || '').trim().toLowerCase()).filter(Boolean));
    }
  ...
  getCategoriaItems(max: number = 3): CategoriaPreviewItem[] {
    return this.getCategoriasObra()
      .slice(0, max)
      .map(value => ({ value, label: this.getCategoriaLabel(value) }));
  }
  ...
  getObraDescripcion(): string {
    return this.obra?.descripcion || this.translationService.getTranslation('mangaPreview.work.no_description');
  }

  get urlReporteObra(): string {
    return this.obra ? `https://minuscreators.com/obra/${this.obra.id}` : '';
  }

  get contextoReporteObra(): string {
    return this.obra ? `Obra de ${this.obra.autor}` : '';
  }
  ...
  private normalizarTipoObra(tipoEntrega?: string): string {
    ...
    const allowed = ['comic', 'manga', 'libro', 'novela', 'artwork'];
